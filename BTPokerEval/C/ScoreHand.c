...
uint32_t scoreHand(uint64_t hand) {
  ...
  if (straightFlushes) {
    ...
    return (uint32_t)(STRAIGHT_FLUSH | (msb(straightFlushes) << VALUE_SHIFT));
  }
  
  // Check quads
  
  // Break into suits
  const uint64_t a = hand & 0xFFFF;
  const uint64_t b = (hand >> 16) & 0xFFFF;
  const uint64_t c = (hand >> 32) & 0xFFFF;
  const uint64_t d = hand >> 48;
  ...
  if (quad) {
    ...
    return (uint32_t)(FOUR_OF_A_KIND | (quad << VALUE_SHIFT) | msb(kickers));
  }
  ...
  if (trips) {
    ...
    if (fullHousePairs) {
      ...
      return (uint32_t)(FULL_HOUSE | (bestTrip << VALUE_SHIFT) | msb(fullHousePairs));
    }
  }
  ...
  if (flushCounts >= 0x5000000000000)
    return (uint32_t)(FLUSH | (ms5b(d) << VALUE_SHIFT));
  if ((flushCounts & 0xFFFF00000000) >= 0x500000000)
    return (uint32_t)(FLUSH | (ms5b(c) << VALUE_SHIFT));
  if ((flushCounts & 0xFFFF0000) >= 0x50000)
    return (uint32_t)(FLUSH | (ms5b(b) << VALUE_SHIFT));
  if ((flushCounts & 0xFFFF) >= 5)
    return (uint32_t)(FLUSH | (ms5b(a) << VALUE_SHIFT));
  ...
  if (straights) {
    ...
    return (uint32_t)(STRAIGHT | (msb(straights) << VALUE_SHIFT));
  }
  ...
  if (trips) {
    ...
    return (uint32_t)(THREE_OF_A_KIND | (trips << VALUE_SHIFT) | ms2b(kickers));
  }
  ...
  if (pairs) {
    ...
    if (lowPairs) {
      ...
      return (uint32_t)(TWO_PAIR | (bestTwoPair << VALUE_SHIFT) | msb(kickers));
    }
    ...
    return (uint32_t)(ONE_PAIR | (bestPair << VALUE_SHIFT) | ms3b(kickers));
  }
